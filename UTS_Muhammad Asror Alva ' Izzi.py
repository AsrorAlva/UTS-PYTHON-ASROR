...
cursorObject.execute("CREATE DATABASE db_V3922050")
...
def insert_data( Id_Barang, Nama_Barang, Harga_Barang, Stok_Awal, Barang_Masuk, Barang_Keluar, Stok_Akhir ):
    sql = "INSERT INTO `DATA STOK BARANG` (`Id Barang`, `Nama Barang`, `Harga Barang`, `Stok Awal`, `Barang Masuk`, `Barang Keluar`, `Stok Akhir`)           VALUES (%s, %s, %s, %s, %s, %s, %s)"
...
def update_data(Id_Barang, Nama_Barang, Harga_Barang, Stok_Awal, Barang_Masuk, Barang_Keluar, Stok_Akhir):
    sql = "UPDATE `DATA STOK BARANG` SET `Nama Barang` = %s, `Harga Barang` = %s, `Stok Awal` = %s, `Barang Masuk` = %s, `Barang Keluar` = %s, `Stok Akhir` = %s WHERE `Id Barang` = %s"
...
def delete_data(Id_Barang):
    sql = "DELETE FROM `DATA STOK BARANG` WHERE `Id Barang` = %s"
    val = (Id_Barang,)
...
def search_data(id_barang):
    sql = "SELECT * FROM `DATA STOK BARANG` WHERE `Id Barang` = %s"
    val = (Id_Barang,)
...
while True:
    ...
    #pilihan 6 "keluar dari program"
        ...
        print("Terimakasih, SEMOGA HARIMU SENIN TERUSS ")
        break
